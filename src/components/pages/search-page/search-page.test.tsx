...
describe('SearchPage Component Test', () => {
    ...
    test('Check when search input changed', () => {
        render(<SearchPage books={bookData} addBook={() => { }} />, { wrapper: BrowserRouter });
        const searchElement = screen.getByLabelText('Search Input', { exact: false }) as HTMLInputElement;
    ...
    test('Check when search input changed and trigger getting data', async () => {
        ...
        const noBooksFoundDiv = screen.queryByText('No books found', { exact: false });
    ...
    test('Check when search input is empty', async () => {
        ...
        const searchHintDiv = screen.queryByText('Please put the message text', { exact: false });
        expect(searchHintDiv).toBeInTheDocument();
    });
});